...
public partial class TestISvcStudyPlan{
	void RegisterPageWeightArg(ITestNode Node){
		...
		R("PageWeightArg_Search_By_UniqName", async(o)=>{
			...
			var page = await SvcStudyPlan.PageWeightArg(userCtx, req, CT.None);
		...
		R("PageWeightArg_Paging_Page0_Page1", async(o)=>{
			var userCtx = MkUserCtx(_ownerA);
			var page0 = await SvcStudyPlan.PageWeightArg(userCtx, new ReqPageWeightArg{
			...
			var page1 = await SvcStudyPlan.PageWeightArg(userCtx, new ReqPageWeightArg{
