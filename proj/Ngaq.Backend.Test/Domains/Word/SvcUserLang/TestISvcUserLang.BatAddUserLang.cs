	...
	void RegisterBatAddUserLang(ITestNode Node){
		...
		R("BatAddUserLang_Should_ThrowPermissionDenied_WhenOwnerMismatch", async(o)=>{
			...
			try{
				...
				AssertThrowsErrItem(ex, KeysErr.Common.PermissionDenied, nameof(ISvcUserLang.BatAddUserLang));
			}
			return NIL;
		});
		...
		R("BatAddUserLang_Should_ThrowDataIllegalOrConflict_OnUniqConflict", async(o)=>{
			...
			try{
				...
				AssertThrowsErrItem(ex, KeysErr.Common.DataIllegalOrConflict, nameof(ISvcUserLang.BatAddUserLang));
			}
			return NIL;
		});
	}
