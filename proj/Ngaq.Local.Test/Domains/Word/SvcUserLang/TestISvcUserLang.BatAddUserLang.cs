...
public partial class TestISvcUserLang{
	void RegisterBatAddUserLang(ITestNode Node){
		...
		register.TesteeFnNames = [nameof(ISvcUserLang.BatAddUserLang)];
		...
		R("BatAddUserLang_Should_ThrowPermissionDenied_WhenOwnerMismatch", async(o)=>{
			...
			try{
				...
				AssertThrowsErrItem(ex, ItemsErr.Common.PermissionDenied, nameof(ISvcUserLang.BatAddUserLang));
			}
			return NIL;
		});
		...
		R("BatAddUserLang_Should_ThrowDataIllegalOrConflict_OnUniqConflict", async(o)=>{
			...
			try{
				await SvcUserLang.BatAddUserLang(MkUserCtx(_ownerA), AsyE(neo), CT.None);
				throw new Exception("BatAddUserLang conflict should throw");
			}
			catch(Exception ex){
				AssertThrowsErrItem(ex, ItemsErr.Common.DataIllegalOrConflict, nameof(ISvcUserLang.BatAddUserLang));
			}
			return NIL;
		});
	}
